...
from Models.XGBOOST.plot import plot_visualization
import gc
import pandas as pd
import numpy as np
from darts.models.forecasting.xgboost import  XGBModel

def load_model_xgb_model(model_name=None):
    model = XGBModel.load(f"{model_name}")
    return model

def evaluation_of_model_xgb_Model(
        model_name=None,
        ts_test=None,
        ts_train=None):
    try:
        ...
        model = load_model_xgb_model(model_name)
        ...
        for window_size in input_window:
            try:
                ...
                # forecasted_series = model.predict(horizon)
                ...
                print('[ERROR] occurred at evaluation_of_model_xgb_Model ',e)
                continue
